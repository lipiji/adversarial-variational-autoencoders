...
class AVAE(object):
    ...
    def define_train_test_funcs(self):
        ...
        for param in self.params:
            gparam = T.grad(cost + loss_g, param)
            gparams.append(gparam)

        lr = T.scalar("lr")
        optimizer = eval(self.optimizer)
        updates_d = optimizer(self.params + self.params_dis, gparams + gparams_d, lr)
        #updates_g = optimizer(self.params_gen, gparams_g, lr)
        
        self.train_d = theano.function(inputs = [self.X, self.Z, lr], outputs = [cost, loss_d, loss_g], updates = updates_d)
        #self.train_g = theano.function(inputs = [self.Z, lr], outputs = [loss_g], updates = updates_g)
